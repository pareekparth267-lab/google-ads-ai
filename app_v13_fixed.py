...
def agent_07_intent_clustering(d: RunCrewRequest) -> dict:
    ...
    return ai_json(
        "You are an intent clustering expert for Google Ads campaign organization. CRITICAL: NEVER include city/location names in keywords. Pure service terms only. Location targeting is handled at the campaign level.",
...
def agent_81_keyword_expander(d: RunCrewRequest) -> dict:
    ...
    return ai_json(
        "You are a keyword expansion and opportunity discovery expert for Google Ads. CRITICAL: NEVER include city/location names in keywords. Use pure service-based terms only. Location is handled by geo-targeting settings.",
